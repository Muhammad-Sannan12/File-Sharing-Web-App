  ...
  useEffect(() => {
    ...
    socket.current.on("connect", () => {
      ...
      if (savedRoomId && savedUserId) {
        socket.current.emit("rejoin-room", {
          roomId: savedRoomId,
          userId: savedUserId,
        });
      }
    ...
    socket.current.on(
      "rejoin-success",
      ({ roomData, roomId, userId, existingFiles, activeUploads }) => {
        ...
        // 1. Handle Finished Files (existingFiles)
        if (existingFiles && existingFiles.length > 0) {
          const initialDownloads = {};
          existingFiles.forEach((file) => {
            // We use uploaderId as the key because your UploadSection
            // prop 'downloadLink' looks for downloadMap[userId]
            initialDownloads[file.uploaderId] = file.fileUrl;
          });
        ...
        // 2. Handle Live Progress for late joiners (activeUploads)
        if (activeUploads) {
          const initialProgress = {};
          Object.entries(activeUploads).forEach(([uId, data]) => {
            initialProgress[uId] = data.percent;
          });
          setProgressMap((prev) => ({
            ...prev,
            ...initialProgress,
